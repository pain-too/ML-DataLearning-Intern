...
# -------------------------- 路径 --------------------------
STROOP_FILE = "/Users/pc/Desktop/longtitudinal_comparison/stroop_多次对比.xlsx"
OUTPUT_DIR = "/Users/pc/Desktop/longtitudinal_comparison/Stroop_LMM_Results"
    ...
    for _, row in df.iterrows():
        ...
        for t in range(9):
            ...
            if pd.isna(s) or s <= 0:
                continue
            long_data.append({
                "id": sid, "group": g, "time": t, "score": float(s)
            })
    return pd.DataFrame(long_data)

# -------------------------- LMM 模型：score ~ time + (1+time|id) --------------------------
def fit_lmm(df, group):
    d = df[df["group"] == group].copy()
    if len(d) < 10:
        return None
    try:
        model = smf.mixedlm("score ~ time", data=d, groups=d["id"], re_formula="1+time")
        res = model.fit(method="lbfgs", maxiter=1000)
        beta = res.params["time"]
        se = res.bse_fe["time"]
        p = 2 * (1 - stats.norm.cdf(abs(beta / se)))
        return {"beta": beta, "p": p, "data": d}
    except:
        return None
...
def plot_figure(results):
    ...
    for ax, (group, res) in zip(axes, results.items()):
        ...
        ax.set_ylabel("Stroop 反应时（秒）", fontsize=12)
        ax.set_xticks(range(9))
        ax.set_ylim(agg["mean"].min() * 0.8, agg["mean"].max() * 1.1)

    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "Stroop_顶刊版_LMM趋势图.png"), dpi=300, bbox_inches="tight")
    plt.close()
    print("✅ Stroop 顶刊版 LMM 趋势图已保存！")

# -------------------------- 主程序 --------------------------
def main():
    df = load_data(STROOP_FILE)
    groups = ["丁香组", "薰衣草组", "雪松组"]
    results = {g: fit_lmm(df, g) for g in groups if fit_lmm(df, g)}
    plot_figure(results)
